  ...
  buyProducts = async (user) => {
    ...
    if (cart.products.length == 0) {
      return {
        status: 405,
        response:
          "Esta accion no esta permitida debido a que su carrito se encuentra vacio",
      };
    }
    ...
    if (failedProducts.length == 0) {
      message =
        "Todos los productos han sido procesados correctamente. Revisa tu email para más informacion";
    } else if (buyContinueProducts.length > 0 && failedProducts.length > 0) {
      message = `Los productos: ${purchasedProductsString} se pudieron procesar correctamente. Los productos: ${failedProductsString} no se pudieron procesar debido a que no tienen stock. Revisa tu email para más informacion`;
    } else {
      message = `Los productos: ${failedProductsString} no se pudieron procesar debido a que no tienen stock.`;
    }
