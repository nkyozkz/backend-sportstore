...
router.get("/failregister", (req, res) => {
  return res.status(401).send({
    status: 401,
    response: "Email ya registrado, ingresa otro",
  });
});
...
router.get("/current", authToken, async (req, res) => {
  ...
  for (let doc of actualDocuments) {
    if (doc.name == "Identificacion") docStatus.identificacion = true;
...
router.post("/updatePassword", async (req, res) => {
  ...
  if (email) {
    ...
    transport.sendMail({
      ...
      to: email,
      subject: "Cambia o restablece tu contraseña",
      html: `
              <div>
...
router.post("/changePassword/:token", authPasswordToken, async (req, res) => {
  ...
  if (contraseña) {
    ...
    if (isValidPassword(user, contraseña)) {
      return res.status(422).send({
        status: 422,
        response: `ingresa una contraseña diferente a la actual`,
      });
    } else {
  ...
  return res.status(422).send({
    response: `ingresa la nueva contraseña en el body como {"contraseña":"nueva contraseña"} y reenvia la peticion`,
  });
});
